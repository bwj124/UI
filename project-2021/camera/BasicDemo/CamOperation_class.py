...
class CameraOperation():
    ...
    def __init__(self, obj_cam, st_device_list, n_connect_num=0, b_open_device=False, b_start_grabbing=False,
                 ...
                 buf_save_image=None, \
                 n_save_image_size=0, n_win_gui_id=0, frame_rate=0, exposure_time=31585, gain=25, action=0):
        ...
        while True:
            ...
            # 如果是彩色且非RGB则转为RGB后显示
                ...
                numArray = CameraOperation.Color_numpy(self, img_buff, self.st_frame_info.nWidth,
                                                       self.st_frame_info.nHeight)
            # TODO: More Processing Logic
            # numArray = self.processor.process(numArray, int(self.action))
    ...
    def Save_jpg(self, buf_cache, fp):
        if (None == buf_cache):
            return
        ...
        if return_code != 0:
            ...
            return
        file_open = open(fp.encode('ascii'), 'wb+')
    ...
    def Save_Bmp(self, buf_cache, fp):
        if (0 == buf_cache):
            return
        self.buf_save_image = None
        self.n_save_image_size = self.st_frame_info.nWidth * self.st_frame_info.nHeight * 3 + 2048
