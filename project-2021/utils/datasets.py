...
from cv2 import waitKey, INTER_LINEAR, destroyAllWindows, CAP_PROP_FRAME_HEIGHT, imwrite, CAP_PROP_BUFFERSIZE, \
    CAP_PROP_FRAME_WIDTH, INTER_AREA, imread, resize, CAP_PROP_FRAME_COUNT, flip, CAP_PROP_FPS, VideoCapture
import numpy as np
from torch import stack, utils, cat, tensor, zeros, from_numpy
...
from utils.general import check_requirements, xywh2xyxy, xywhn2xyxy, xyxy2xywhn, \
...
def create_dataloader(path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
    ...
    sampler = utils.data.distributed.DistributedSampler(dataset) if rank != -1 else None
    loader = utils.data.DataLoader if image_weights else InfiniteDataLoader
...
class InfiniteDataLoader(utils.data.dataloader.DataLoader):
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.fps[i] = max(cap.get(CAP_PROP_FPS) % 100, 0) or 30.0  # 30 FPS fallback
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        labels_out = zeros((nl, 6))
        if nl:
            labels_out[:, 1:] = from_numpy(labels)

        # Convert
        img = img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        img = np.ascontiguousarray(img)

        return from_numpy(img), labels_out, self.img_files[index], shapes
    ...
    def collate_fn(batch):
        ...
        return stack(img, 0), cat(label, 0), path, shapes
    ...
    def collate_fn4(batch):
        ...
        ho = tensor([[0., 0, 0, 1, 0, 0]])
        wo = tensor([[0., 0, 1, 0, 0, 0]])
        s = tensor([[1, 1, .5, .5, .5, .5]])  # scale
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                ...
                im = cat((cat((img[i], img[i + 1]), 1), cat((img[i + 2], img[i + 3]), 1)), 2)
                l = cat((label[i], label[i + 1] + ho, label[i + 2] + wo, label[i + 3] + ho + wo), 0) * s
            img4.append(im)
            label4.append(l)
        ...
        return stack(img4, 0), cat(label4, 0), path4, shapes4
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        if r != 1:  # if sizes are not equal
            img = resize(img, (int(w0 * r), int(h0 * r)),
                         interpolation=INTER_AREA if r < 1 and not self.augment else INTER_LINEAR)
        ...
        if im_file.suffix[1:] in IMG_FORMATS:
            ...
            if Path(lb_file).exists():
                ...
                for j, x in enumerate(lb):
                    ...
                    assert imwrite(str(f), im[b[1]:b[3], b[0]:b[2]]), f'box failure in {f}'
