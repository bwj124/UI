...
from math import pi
from matplotlib.pyplot import figure, legend, subplots
from numpy import stack, concatenate, interp, sum, zeros, int16, argsort, unique, maximum, trapz, where, nan, flip, \
    minimum, linspace
from torch import stack, min, max, cat, pow, where, atan, no_grad
...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=()):
    ...
    # Find unique classes
    unique_classes = unique(target_cls)
...
def compute_ap(recall, precision):
    ...
    mrec = concatenate(([0.], recall, [recall[-1] + 0.01]))
    mpre = concatenate(([1.], precision, [0.]))

    # Compute the precision envelope
    mpre = flip(maximum.accumulate(flip(mpre)))
    ...
    if method == 'interp':
        ...
        ap = sum((mrec[i + 1] - mrec[i]) * mpre[i + 1])  # area under curve
...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):
        ...
        x = where(iou > self.iou_thres)
        if x[0].shape[0]:
            matches = cat((stack(x, 1), iou[x[0], x[1]][:, None]), 1).cpu().numpy()
            if x[0].shape[0] > 1:
    ...
    def plot(self, normalize=True, save_dir='', names=()):
        try:
            ...
            with catch_warnings():
                simplefilter('ignore')  # suppress empty matrix RuntimeWarning: All-NaN slice encountered
    ...
    if GIoU or DIoU or CIoU:
        ...
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if DIoU:
                ...
                v = (4 / pi ** 2) * pow(atan(w2 / h2) - atan(w1 / h1), 2)
                with no_grad():
                    alpha = v / (v - iou + (1 + eps))
...
def plot_pr_curve(px, py, ap, save_dir='pr_curve.png', names=()):
    ...
    fig, ax = subplots(1, 1, figsize=(9, 6), tight_layout=True)
    py = stack(py, axis=1)
