...
from cv2 import copyMakeBorder, INTER_LINEAR, getRotationMatrix2D, cvtColor, COLOR_YUV2BGR, COLOR_YUV2RGB, bitwise_and, \
    LUT, createCLAHE, warpAffine, COLOR_BGR2HSV, COLOR_RGB2YUV, split, warpPerspective, drawContours, merge, \
    COLOR_HSV2BGR, equalizeHist, resize, FILLED, COLOR_BGR2YUV, flip, BORDER_CONSTANT
from math import tan, pi
from numpy import mod, concatenate, arange, float32, zeros, eye, random, array, int32, ones, maximum, uint8, clip, \
    append
...
class Albumentations:
    ...
    def __init__(self):
        ...
        try:
            ...
            info(colorstr('albumentations: ') + ', '.join(f'{x}' for x in self.transform.transforms if x.p))
        except ImportError:  # package not installed, skip
            pass
        except Exception as e:
            info(colorstr('albumentations: ') + f'{e}')

    def __call__(self, im, labels, p=1.0):
        if self.transform and random() < p:
...
def augment_hsv(im, hgain=0.5, sgain=0.5, vgain=0.5):
    ...
    if hgain or sgain or vgain:
        r = random.uniform(-1, 1, 3) * [hgain, sgain, vgain] + 1  # random gains
...
def replicate(im, labels):
    ...
    for i in s.argsort()[:round(s.size * 0.5)]:  # smallest indices
        ...
        yc, xc = int(uniform(0, h - bh)), int(uniform(0, w - bw))  # offset x, y
        x1a, y1a, x2a, y2a = [xc, yc, xc + bw, yc + bh]
        im[y1a:y2a, x1a:x2a] = im[y1b:y2b, x1b:x2b]  # im4[ymin:ymax, xmin:xmax]
        labels = append(labels, [[labels[i, 0], x1a, y1a, x2a, y2a]], axis=0)
...
def random_perspective(im, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    # Shear
    S = eye(3)
    S[0, 1] = tan(uniform(-shear, shear) * pi / 180)  # x shear (deg)
    S[1, 0] = tan(uniform(-shear, shear) * pi / 180)  # y shear (deg)
...
def copy_paste(im, labels, segments, p=0.5):
    ...
    if p and n:
        ...
        for j in sample(range(n), k=round(p * n)):
...
def cutout(im, labels, p=0.5):
    ...
    if random() < p:
        ...
        for s in scales:
            mask_h = randint(1, int(h * s))  # create random masks
            mask_w = randint(1, int(w * s))

            # box
            xmin = max(0, randint(0, w) - mask_w // 2)
            ymin = max(0, randint(0, h) - mask_h // 2)
            xmax = min(w, xmin + mask_w)
            ymax = min(h, ymin + mask_h)

            # apply random color mask
            im[ymin:ymax, xmin:xmax] = [randint(64, 191) for _ in range(3)]
